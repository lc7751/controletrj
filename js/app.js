...
(function (TRJ) {
  ...
  App.loadAll = async function () {
    U.loading(true);
    try {
      var res = await Promise.all([TRJ.api.getConfig(), TRJ.api.getTasks(), TRJ.api.getIncidents()]);
      var config = res[0].config || {};
      var rawTasks = res[1].rows || [];
      var rawInc = res[2].rows || [];
  ...
  // recarrega só incidentes (após upload/alteração de status)
  App.reloadIncidents = async function () {
    var res = await TRJ.api.getIncidents();
    var rawInc = res.rows || [];
