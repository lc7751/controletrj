...
(function (TRJ) {
  ...
  // O Genesis concatena [nota do técnico] + " | " + [dados do EVE: Falha | Nome da BSC: ...]
  // quando um incidente tem nota manual. Para incidentes sem nota, td9 fica só com dados EVE.
  // Nós queremos mostrar APENAS a nota do técnico.
  function limparDetalheGenesis(texto) {
    if (!texto) return null;
    var t = texto;
    // 1. Remove linha "Última atualização do evento: ..."
    t = t.replace(/\n?[UÚÃ\u009a][lú]?ltima\s+atualiza[\u00e7\u00e3][\u00e3o]?\s+do\s+evento[\s\S]*$/i, '').trim();
    // 2. Remove bloco " | ... | Nome da BSC: ..." (EVE alarm appended by Genesis JS)
    // Inclui o tipo de falha que aparece antes de "Nome da BSC:" ex: "| Climatização | Nome da BSC:"
    ...
    // 3. Se o que sobrou é puro EVE (sem menção a técnico, GMG, TSK, etc.) → descartar
    var temTecnico = /[Tt][eé\u00c3\u0089][ÉĐc\u00e9n]|GMG\b|K\d\b|TSK\d|IDENTIFIC/i.test(t) ||
                     /^[\d]{2}\/[\d]{2}/.test(t.trim()); // começa com "02/07"
    var ehEvePuro = /^(Indisponibilidade|N[\u00e3a]o\s+Classificado|Tx\s+Terceira|Climatiza|Energia\s+AC|Não\s+Classificado)/i.test(t.trim());
