...
(function (TRJ) {
  TRJ.pages = TRJ.pages || {};
  var U = TRJ.ui, C = TRJ.constants, Comp = TRJ.compute, G = TRJ.genesis, FS = TRJ.files;
  ...
  function chave(r) { return ((r.site || '') + '|' + (r.enderecoId || '')).toUpperCase(); }

  async function mudarStatus(row, status, app) {
    try {
      U.loading(true);
      // atualiza o incidente correspondente na memória/navegador
      var raw = FS.getIncidents();
      var alvo = chave(row);
      var achou = false;
      raw.forEach(function (it) { if (chave(it) === alvo) { it.statusTrat = status; achou = true; } });
      if (!achou) raw.forEach(function (it) { if ((it.site || '').toUpperCase() === (row.site || '').toUpperCase()) it.statusTrat = status; });
      FS.setIncidents(raw, Object.assign({}, (FS.getMeta().inc || {}), { em: new Date().toISOString() }));
